<template>
	<div class="top_view">
		...
		<div class="projectTitle">网上订餐系统</div>
...
<script setup>
	...
	const getSession = () => {
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('sessionTable') == 'shangjia'){
				context?.$toolUtil.storageSet('adminHeadportrait',res.data.data.yingyezhizhao)
			}
		})
	}
	// 退出登录
